...
test("HTTP authentication, CSRF, memberships, revision checks, and sign-out protect shared data", async () => {
  ...
  try {
    ...
    const reordered = structuredClone(latest.workspace!);
    reordered.projects[0].roadmap.tasks = moveStep("runtime-access", "up", reordered.projects[0].roadmap.tasks);
    const reorderSave = await request("/api/workspace", "PUT", { revision: latest.revision, workspace: reordered }, cookie);
    assert.equal(reorderSave.status, 200, await reorderSave.text());
    latest = await (await request("/api/workspace", "GET", undefined, cookie)).json() as SharedSnapshot;
    assert.deepEqual(childrenOf("network", latest.workspace!.projects[0].roadmap.tasks).map(t => t.id), ["runtime-access", "runner-access"]);
    assert.ok(same(latest.workspace, reordered));
